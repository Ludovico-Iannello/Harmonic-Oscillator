	...
	parameter (measures=500000)
	real t1,t2
	real*8 var_y2,var_Dy2,var_corr,lattice,y2,Dy2
	real*8 O_mean_1(measures)
	real*8, dimension(:), allocatable :: field, O_arr
...
CC====================================
	...
	i_decorrel=20        !!updating fra una misura e l'altra
	i_term=1000000        !!passi di termalizzazione
	beta_omega=20
	...
	do i=1,20
		...
		write(*,*) 'eta', eta
		d_metro=2*sqrt(eta)       !!parametro del metropolis
...
c*****************************************************************
      	real*8, dimension(Npassi) :: field
      	real*8, dimension(int(Npassi/2)) :: O_arr
...
C=================================================================
	subroutine bootstrap(x,dx)
        ...
        parameter (k=1000,N=500000,p=200)
        real*8 dx,media_x
        real*8 sim_arr(k)      		    !!media  simulazioni
        real*8 a(N),x(N)        !!a array provvisorio
...
c=================================================================
        ...
        parameter (measures=500000,k=1000,p=100)
        real*8 x,m,dm,u,dev,y2,Dy2
        real*8 a(k)
        common/arr/y2(measures),Dy2(measures)
        common/various/eta,d_metro
        x=0
